...
<!-- <header class={"sticky z-[2] top-0 duration-200 px-4 flex items-center justify-between border-b border-solid " + (y > 0 ? " py-4 bg-slate-950 border-violet-950" : " py-6 bg-transparent border-transparent")}> -->
  <header class={"sticky z-[2] top-0 duration-200 px-4 flex items-center justify-between border-b border-solid " + (y > 0 && !isOpen ? " py-4 bg-blue-200 dark:bg-slate-950 border-violet-900 dark:border-violet-950" : " py-6 bg-transparent border-transparent")}>
  ...
  <div class="flex items-center justify-between w-full">
    ...
    <div class="hidden ml-auto md:flex">
      <ul class="flex items-center gap-4 uppercase">
        {#if !$page.error && $page.url.pathname === '/'}
          {#each tabs as tab, index}
            <li>
              <a href={tab.link} class="tracking-widest duration-200 hover:text-violet-600 dark:hover:text-violet-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap" target={index === 3 ? "_blank" : ""} >
                <span>{tab.name}</span>
              </a>
            </li>
          {/each}
      ...
      <li class="flex items-center justify-center">
        <a href="https://www.linkedin.com/in/elena-shatalova/" target="_blank"  class="font-medium blueShadowLight dark:blueShadow relative overflow-hidden px-5 py-2 group rounded-full bg-white text-violet-600 dark:text-slate-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap">
      ...
      <li>
        <button on:click={toggleTheme} class="w-9 h-9 flex items-center justify-center rounded-full bg-white hover:bg-slate-200 transition duration-300 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50" aria-label={$theme === 'light' ? 'Switch to Dark Theme' : 'Switch to Light Theme'}>
          {#if $theme === 'light'}
    ...
    <button 
      ...
      class:open={isOpen}
      id="btn"
      class={`z-[3] block md:hidden hamburger focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 ${isOpen && y > 0 ? 'top-6' : ''}`}
  ...
  <div bind:this={menu} class:flex={!isHidden} class:hidden={!isFlex} id="menu" class="fixed inset-0 z-[2] hidden self-end w-svh h-full m-h-screen opacity-90 bg-blue-200 dark:bg-slate-950 md:hidden before:opacity-100 before:absolute before:h-10 before:w-screen before:inset-x-0 top-0 before:bg-blue-200  dark:before:bg-slate-950 after:opacity-100 after:absolute after:h-screen after:w-screen afetr:inset-x-0 bottom-0 after:bg-blue-200 dark:after:bg-slate-950">
        ...
        {#each tabs as tab, index}
          <li>
            <div class="w-full py-3 text-center">
              <a href={tab.link} class="block duration-200  hover:text-violet-600 dark:hover:text-violet-400 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap" target={index === 3 ? "_blank" : ""}>
                <span>{tab.name}</span>
              </a>
            </div>
          </li>
    ...
    <div class="flex items-center justify-center w-full px-5 py-3 gap-4">
      <li>
        <a href="https://www.linkedin.com/in/elena-shatalova/" target="_blank" class="font-medium block blueShadowLight dark:blueShadow relative overflow-hidden px-5 py-2 group rounded-full bg-white text-violet-600 dark:text-slate-950 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap">
          <div class="absolute top-0 right-full w-full h-full bg-violet-600 dak:bg-violet-400 opacity-20 group-hover:translate-x-full z-[0] duration-200"></div>
      ...
      <li>
        <button on:click={toggleTheme} class="w-9 h-9 flex items-center justify-center rounded-full bg-white hover:bg-slate-200 transition duration-300 ease-in-out focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50">
          <!-- {#if themeLoaded} -->
          {#if $theme === 'light'}
            <img class="w-5 h-5 transition duration-300 ease-in-out" src="/images/moon-to-sun.svg" alt="Light Theme">
          {:else}
