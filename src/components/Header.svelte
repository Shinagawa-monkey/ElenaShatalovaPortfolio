...
<!-- <header class={"sticky z-[2] top-0 duration-200 px-4 flex items-center justify-between border-b border-solid " + (y > 0 ? " py-4 bg-slate-950 border-violet-950" : " py-6 bg-transparent border-transparent")}> -->
  ...
  <!-- Mobile Menu -->
    <div bind:this={menu} class:flex={!isHidden} class:hidden={!isFlex} id="menu" class="fixed inset-0 z-[2] hidden self-end w-full h-full m-h-screen opacity-90 bg-slate-950 md:hidden before:absolute before:h-32 before:w-screen before:inset-x-0 top-0 before:opacity-90 before:bg-slate-950 after:absolute after:h-screen after:w-screen afetr:inset-x-0 bottom-0 after:opacity-90 after:bg-slate-950">
