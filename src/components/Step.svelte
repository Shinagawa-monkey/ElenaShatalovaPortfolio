...
<div style="height: {height}" class="p-4 sm:p-6 md:p-8 flex flex-col gap-4 rounded-lg border border-solid border-violet-600 dark:border-violet-700 text-center cursor-default group hover:border-violet-800 hover:shadow-inner dark:hover:shadow-none dark:hover:border-violet-400 duration-200 transition-height">
  ...
  <div class="text-left mt-4">
    ...
    <div class="flex flex-col gap-2">
      <ul class="flex flex-wrap gap-2">
        {#each step.technologies.split(', ') as tag}
          ...
          <li class="bg-violet-600 dark:bg-violet-700 text-white px-3 py-1 rounded-full shadow-md text-sm sm:text-base whitespace-nowrap">{tag}</li>
        {/each}
      </ul>
    </div>
    {/if}
    ...
    <button on:click={toggleShowMore} aria-label={showMore ? "Show Less" : "Read More"} class="font-medium text-base md:text-lg poppins relative overflow-hidden px-3 py-1.5 rounded-full bg-violet-500 text-slate-950 cursor-pointer hover:bg-violet-600 duration-200 group/expandBtn focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap" tabindex={$isMenuOpen ? -1 : 0}>
    ...
    <a href={step.href} target="_blank" aria-label={"Go to " + step.name} class="font-medium text-base md:text-lg poppins relative overflow-hidden px-3 py-1.5 rounded-full bg-white dark:bg-gray-100 text-slate-950 cursor-pointer hover:bg-gray-100 duration-200 group/GoToBtn focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-600 dark:focus-visible:ring-violet-400 focus-visible:ring-opacity-50 whitespace-nowrap" tabindex={$isMenuOpen ? -1 : 0}>
